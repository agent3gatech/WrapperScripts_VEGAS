...
if [ -e ${CALIBDIROUT}/${LASERRUN}.root ]; then
    ...
    EXECUTE="${SCRIPTDIR}/laser.sh ${SRC} ${LASERRUN} ${CALIBDIRIN} ${CALIBDIROUT} ${STAGE1CONFIGFILE} ${ROOT} ${VEGASBASE}"
    echo "${EXECUTE}"
    ${EXECUTE}
fi
...
for ((i=0;i<${#RUNS[@]};i++)); do
  ...
  else if [ -f ${CALIBDIRIN}/${RUNS[${i}]}.${SRC}.cvbf ] || [ -f ${CALIBDIRIN}/${RUNS[${i}]}.vbf ]; then
       #Add run to list of runs that will be calibrated	
       RUNSTOCALIBRATE=("${RUNSTOCALIBRATE[@]}" "${RUNS[${i}]}" )
  else
...
echo "${#RUNSTOCALIBRATE[@]}"
echo "${RUNSTOCALIBRATE}"

if [ "${RUNSTOCALIBRATE}" != "" ]; then
 for J in ${RUNSTOCALIBRATE[@]}; do
     ARGUMENTS="-A GT-aotte6 -d ${CALIBDIROUT} -e log/err-${J} -o log/out-${J} -N VTScalib -t ${J} -v arg1=${SRC},arg2=${MC_CARE},arg3=${LASERRUN},arg4=${CALIBDIRIN},arg5=${CALIBDIROUT},arg6=${STAGE1CONFIGFILE},arg7=${STAGE2CONFIGFILE},arg8=${SCRIPTDIR},arg9=${ROOT},arg10=${VEGASBASE},arg11=${SCRATCHDIR},arg12=${MCNOISELVL} ${EXECUTABLE}"
