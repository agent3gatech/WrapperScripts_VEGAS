...
if [ "${MC}" = "true" ]; then #MC stuff
	...
	echo "
           ${SCRIPTDIR}/submit_stage1_2.sh ${SCRATCH} ${QUEUE} ${ROOT} ${VEGASBASE} ${SCRIPTDIR} ${SRC} ${LASERRUN} ${MC_CARE} ${CALIBDIRIN} ${CALIBDIROUT} ${STAGE1CONFIGFILE} ${STAGE2CONFIGFILE} ${level} $RUNS"    
	${SCRIPTDIR}/submit_stage1_2.sh ${SCRATCH} ${QUEUE} ${ROOT} ${VEGASBASE} ${SCRIPTDIR} ${SRC} ${LASERRUN} ${MC_CARE} ${CALIBDIRIN} ${CALIBDIROUT} ${STAGE1CONFIGFILE} ${STAGE2CONFIGFILE} ${level} "$RUNS"
    ...
    for ((i=0;i<${#ALLRUNS[@]};i++)); do
		...
		if [ ! -f ${CALIBDIRIN}/${RUNS}.${SRC}.cvbf -a ! -f ${CALIBDIRIN}/${RUNS}.vbf ]; then
		...
		for ((l=i+1;l<${#ALLRUNS[@]};l++)); do
			    ...
			    DAY=${STRING[ ${#STRING[@]}-2  ]}
					...
					if [ "${DAY}" = "${CURRENTDAY}" ]; then
					    ...
					    if [ ! -f ${CALIBDIRIN}/${RUN}.${SRC}.cvbf -a ! -f ${CALIBDIRIN}/${RUN}.vbf ]; then
   ...
   #next line calls calibration for the day
		echo "
                   ${SCRIPTDIR}/submit_stage1_2.sh ${SCRATCH} ${QUEUE} ${ROOT} ${VEGASBASE} ${SCRIPTDIR} ${SRC} ${LASERRUN} ${MC_CARE} ${CALIBDIRIN} ${CALIBDIROUT} ${STAGE1CONFIGFILE} ${STAGE2CONFIGFILE} $RUNS"
